...
join_qc <- full_join(tibble(assembly_accession = tree$tip.label, in_tree = TRUE),
                    metadata %>% mutate(in_metadata = TRUE), by = "assembly_accession") %>%
...
if (nrow(metadata) != 72 || length(tree$tip.label) != 72 ||
    sum(join_qc$qc_status == "MATCH") != 72 || nrow(join_qc) != 72) stop("Tree/metadata join is not exactly 72:72")

plot_data <- metadata %>% rename(label = assembly_accession)
...
for (group in c("Reproductive", "Bacteraemia")) {
  keep <- metadata %>% filter(reproductive_bacteraemia_category == group) %>% pull(assembly_accession)
...
caption <- paste(
  "Ten-locus concatenated housekeeping-gene maximum-likelihood phylogeny.",
...
writeLines(caption, file.path(output_dir, "captions", "phylogeny_10_locus_caption.txt"))
message("Figures generated; tree tips and metadata matched exactly 72:72.")
