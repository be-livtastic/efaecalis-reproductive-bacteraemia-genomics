...
- `local_archive/`: ignored legacy inputs and large outputs; not part of the published workflow.

`data/raw/`, `data/interim/`, bulk annotations and current phylogeny work are
local-only and regenerated by stage scripts. `local_archive/` preserves legacy
material and is not an input to the active pipeline.
...
AMRFinderPlus analysis, Prokka annotation and the documented nine-locus
phylogeny have been run. Panaroo, recombination filtering and a core-genome
phylogeny are not yet present. Earlier single-gene `rpoB` and failed/interrupted
attempts have been removed. See `docs/workflow/analysis_workflow.md`.
...
6. Annotate genomes with `bash scripts/04_annotation/run_prokka.sh`.
7. Run the remaining scripts from their documented stage in numerical order.
8. Review input and output paths in `config/paths.example.yml`.
